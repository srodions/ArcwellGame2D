...
{
	obj_sprites[0] = IMG_LoadTexture(pRenderer, "torch.png");
	obj_sprites[1] = IMG_LoadTexture(pRenderer, "decoration.png");
	obj_sprites[2] = IMG_LoadTexture(pRenderer, "chest.png");
}
...
location_t L_LocationInit(SDL_Renderer* pRenderer, obj_manager_t* pObjManager)
{
	char* fullData = (char*) SDL_LoadFile("location.dat", NULL);
	if (fullData == NULL) printf("Location load error!\n");

	char* headerData = (char*) SDL_LoadFile("location.json", NULL);
	...
	location_t location = {
		.tileMap = IMG_LoadTexture(pRenderer, "tiles.png"),
