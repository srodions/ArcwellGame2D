...
SDL_Texture* L_InitTexture(SDL_Renderer* pRenderer, FILE* arcFile, const char* textureName, arcf_header_t* pHeader, arcf_entry_t* pTable, uint32_t* outSize);

/* --- IMPLEMENTATIONS --- */
#if defined(STB_ARCLOADER_IMPLEMENTATION)
...
SDL_Texture* L_InitTexture(SDL_Renderer* pRenderer, FILE* arcFile, const char* textureName, arcf_header_t* pHeader, arcf_entry_t* pTable, uint32_t* outSize)
{
	...
	void* textureData = L_LoadLump(arcFile, textureName, pHeader, pTable, outSize);
	if (textureData)
	{
		SDL_RWops* rw = SDL_RWFromMem(textureData, *outSize);
...
#endif /* STB_ARCLOADER_IMPLEMENTATION */
